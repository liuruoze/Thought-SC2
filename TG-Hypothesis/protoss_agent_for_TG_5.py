...
from strategy.protoss_action import ProtossAction
from unit.units import Army

import unit.protoss_unit as P

# This is the TG-5 (maybe new TG-4) module of original thought-game
# compared to TG-0, it removes following modules:
# N_8:Resource limit
# N_7:Resource efficiency
# N_6:Production queue
# N_5:Production time
# N_4:Technology tree
# N_3:Population satisfaction
# N_1:Resource satisfaction


MIN_PRODUCE_LEN = 1  # default is 5, this is represent module N_6:Production queue
...
class MiniAgent(A):
    ...
    def fullfill_creature_condition(self, unit):
        if 1: # default is self.mineral >= unit.mineral_price and self.gas >= unit.gas_price: , this is represent module N_1:Resource satisfaction
            # default is self.food_cup >= self.food_used + unit.food_used, this is represent module N_3:Population satisfaction
            if 1 and 1:  # default is self.fullfill_technology(unit): , this is represent module N_4:Technology tree
                return True
        else:
            return False
    ...
    def fullfill_building_condition(self, unit):
        if 1: # default is self.mineral >= unit.mineral_price and self.gas >= unit.gas_price: , this is represent module N_1:Resource satisfaction
            if 1:  # # default is self.fullfill_technology(unit): , this is represent module N_4:Technology tree
                return True
        else:
            return False
    ...
    def update_population_progress(self):
        for b in self.production_building_list:
            if not b.queue.isEmpty():
                ...
                if self.fullfill_food_condition(unit) and (True) : # default is unit.progress >= unit.build_time, this is represent module N_5:Production time
                    u = b.queue.dequeue()
    ...
    def update_building_progress(self):
        for b in self.on_building_list:
            ...
            if (True):  # default is b.progress >= b.build_time, this is represent module N_5:Production time
    ...
    def update_resources(self):
        # update mineral
        idle_worker_for_mineral = sys.maxsize  # default is 20, this is represent module N_7:Resource efficiency
        max_mineral_for_one_base = sys.maxsize   # default is 10000, this is represent module N_8:Resource limit
        max_gas_for_one_base = sys.maxsize  # default is 3000, this is represent module N_8:Resource limit
