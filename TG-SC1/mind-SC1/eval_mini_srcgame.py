...
flags.DEFINE_string("restore_model_path", "./model/20190121-212908_mini/", "path for restore model")
...
flags.DEFINE_string("hostname", "58.192.50.50", "remote ip of sc server")
flags.DEFINE_integer("sc_port", 11111, "remote port of sc server")
flags.DEFINE_integer("frame_skip", 48, "more larger, the training speed is larger")
...
def Worker(index, update_game_num, Synchronizer, cluster, model_path, initial_sc_port):
    ...
    for i in range(THREAD_NUM - 1):
        t = threading.Thread(target=run_thread, args=(agents[i], game_num, Synchronizer, FLAGS.difficulty, initial_sc_port))
        initial_sc_port += 1
        threads.append(t)
        t.daemon = True
        t.start()
    ...
    run_thread(agents[-1], game_num, Synchronizer, FLAGS.difficulty, initial_sc_port)
...
def _main(unused_argv):
    ...
    initial_sc_port = FLAGS.sc_port

    procs = []
    for index in range(PARALLEL):
        p = mp.Process(name="Worker_%d" % index, target=Worker, args=(index, per_update_num, Synchronizer, Cluster, model_path, initial_sc_port))
        initial_sc_port += THREAD_NUM
        procs.append(p)
        p.daemon = True
        p.start()
        time.sleep(1)
