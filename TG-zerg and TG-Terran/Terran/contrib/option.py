...
def attack_step(agent, pos_index=None):
    ...
    # if agent.obs.raw_observation.observation.player_common.army_count > 30:ss
    if pos_index is None or pos_index == 0:
        ...
        # agent.safe_action(C._ATTACH_M, C._ARMY_INDEX, [C._QUEUED, [34, 42]])
        # agent.safe_action(C._ATTACH_M, C._ARMY_INDEX, [C._QUEUED, [46, 52]])
        x = 1
...
def gather_resource(agent, resource='gas'):
    ...
    if resource == 'gas':
        ...
        if gas_pos and drone_pos:
            agent.select(C._SELECT_POINT, C._SCV_TYPE_INDEX, [C._CLICK, drone_pos])
            agent.safe_action(C._HARVEST_S, C._SCV_TYPE_INDEX, [C._NOT_QUEUED, gas_pos])
    if resource == 'mineral':
        ...
        if mineral_pos and drone_pos:
            agent.select(C._SELECT_POINT, C._SCV_TYPE_INDEX, [C._CLICK, drone_pos])
            agent.safe_action(C._HARVEST_S, C._SCV_TYPE_INDEX, [C._NOT_QUEUED, mineral_pos])
...
def train_worker(agent, building_pos, train_action, click=C._CLICK):
    ...
    agent.select(C._SELECT_POINT, C._COMMANDCENTER_TYPE_INDEX, [click, building_pos])
    # agent.select(C._SELECT_LARVA, C._LARVA_TYPE_INDEX, [])
    agent.safe_action(train_action, C._COMMANDCENTER_TYPE_INDEX, [C._NOT_QUEUED])


def build_by_idle_worker(agent, build_action, build_pos):
    if C._SELECT_WORKER in agent.obs.observation["available_actions"]:
        agent.select(C._SELECT_WORKER, C._SCV_TYPE_INDEX, [[0]])
    else:
        agent.select(C._SELECT_POINT, C._SCV_TYPE_INDEX, [C._CLICK, select_unit(agent)])
    agent.safe_action(build_action, C._SCV_TYPE_INDEX, [C._NOT_QUEUED, build_pos])
    agent.safe_action(C._HARVEST_S, C._SCV_TYPE_INDEX, [C._QUEUED, U.get_back_pos(agent.obs, agent.env.game_info)])
...
def train_army(agent, train_action):
    ...
    barracks_pos = T.world_to_screen_pos(agent.env.game_info, barracks.pos, agent.obs)
    agent.select(C._SELECT_POINT, C._BARRACKS_TYPE_INDEX, [C._DBL_CLICK, barracks_pos])
...
def select_unit(agent, unit_type=C._SCV_TYPE_INDEX):
