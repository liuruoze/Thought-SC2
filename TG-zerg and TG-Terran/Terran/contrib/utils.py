...
def get_valid_mask_screen(obs, size=None, show=False):
    screen_unit_type = obs.observation["screen"][_UNIT_TYPE]
    screen_unit = (screen_unit_type != 0).astype('int')
...
def find_gas(obs, index):
    ...
    unit_set = obs.raw_observation.observation.raw_data.units
    for unit in unit_set:
        if unit.unit_type == C._REFINERY_TYPE_INDEX:
...
def is_Refinery_on_gas(obs, gas):
    unit_set = obs.raw_observation.observation.raw_data.units
    for unit in unit_set:
        if unit.unit_type == C._REFINERY_TYPE_INDEX:
...
def get_mineral_drone(obs):
    ...
    for unit in unit_set:
        if unit.unit_type == C._SCV_TYPE_INDEX and unit.is_on_screen is True:
            buff = unit.buff_ids
            # [274] gas
            # [271] mineral
...
def get_best_gateway(obs):
    ...
    for unit in unit_set:
        if unit.unit_type == C._BARRACKS_TYPE_INDEX and unit.build_progress == 1:
...
def get_input(obs):
    ...
    index_list = [C._REFINERY_TYPE_INDEX, C._SUPPLYDEPOT_INDEX, C._BARRACKS_TYPE_INDEX,
                  C._SCV_TYPE_INDEX, C._MARINE_TYPE_INDEX, C._REAPER_TYPE_INDEX]
    high_input[11:17] = get_unit_num_array(obs, index_list)

    high_input[17] = get_attack_num(obs, [C._MARINE_TYPE_INDEX, C._REAPER_TYPE_INDEX])
    ...
    # #################################  pop num  #################################################
    base = find_unit(obs, C._COMMANDCENTER_TYPE_INDEX)
    # number of probe for mineral and the ideal num
    if base:
