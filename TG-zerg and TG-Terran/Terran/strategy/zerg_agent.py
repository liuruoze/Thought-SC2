...
class ZergAction(Enum):
    ...
    Build_roach = 8
    Build_extractor = 9
    Build_spawningpool = 10
    Build_evolutionchamber = 11
    Build_spinecrawler = 12
    Build_roachwarren = 13
    Gather_gas = 14
    Gather_mineral = 15
...
class Zerg(A):
    ...
    def init_features(self):
        ...
        self.evolutionchamber_num = 0
        self.queen_num = 0
        self.zergling_num = 0
        self.roach_num = 0
        self.spinecrawler_num = 0
    ...
    def obs(self):
        ...
        simple_input[15] = self.evolutionchamber_num
        simple_input[16] = self.queen_num
    ...
    def fullfill_creature_condition(self, unit, num):
        if self.mineral >= unit.mineral_price * num and self.gas >= unit.gas_price * num:
            if type(unit) == Z.Overlord and self.larva_num > 0:
                return True
            elif self.food_cap >= self.food_used + unit.food_used * num and self.fullfill_technology(unit):
    ...
    def build_units(self, unit_type):
        ...
        if num == 0:
            return
        if not self.fullfill_creature_condition(unit, num):
            return
...
class DummyZerg(A):
    ...
    def step_auto(self, verbose=False):
        if self.env.all_steps % 5 == 1 and self.difficulty >= 2:
            ...
            if self.env.all_steps >= 35 and self.difficulty >= 7:
                self.add_unit(Z.Zergling(), 2)
            if self.env.all_steps >= 45 and self.difficulty >= 9:
                self.add_unit(Z.Roach(), 1)
            if self.env.all_steps >= 55 and self.difficulty >= 11:
                self.add_unit(Z.Roach(), 1)
            if self.env.all_steps >= 65 and self.difficulty >= 13:
                self.add_unit(Z.Hydralisk(), 1)
    ...
    def get_power(self):
        self.add_unit(Z.Zergling(), 6, u_type='army')
        self.add_unit(Z.Roach(), min(self.difficulty - 1, 10), u_type='army')        
