...
import param as P
import zerg_source_agent as mini_source_agent
...
from strategy.zerg_agent import DummyZerg
...
# flags.DEFINE_string("restore_mini_path", "./model/20190121-212908_mini/", "path for restore mini")
# flags.DEFINE_bool("restore_mini", True, "Whether to restore old model")ss
...
flags.DEFINE_integer("port_num", 59211, "the start port to create distribute tf")
...
flags.DEFINE_string("log_file", "./result/19-02-23_ZvZ_Simple64_log_2.txt", "log file to save data")
...
Difficulty_list = [FLAGS.difficulty]
Reward_list = [1, 0, -1]
...
def run_thread(agent, Synchronizer):
    ...
    for difficulty in Difficulty_list:
        with sc2_env.SC2Env(
                ...
                visualize=False, game_steps_per_episode=600 * 22.4) as env:
...
def Worker(index, update_game_num, Synchronizer, cluster, model_path):
    ...
    for i in range(THREAD_NUM):
        agent = mini_source_agent.SourceAgent(index=i, global_buffer=global_buffer, net=Net,
...
def Parameter_Server(Synchronizer, cluster, log_path, model_path):
    ...
    agent = mini_source_agent.SourceAgent(index=-1, net=Net, restore_model=FLAGS.restore_model, rl_training=FLAGS.training)
