...
from strategy.zerg_agent import Zerg, DummyZerg
...
FLAGS = flags.FLAGS
flags.DEFINE_string("restore_model_path", "./model/20190222-180620_mini/", "path for restore model")
...
def run_thread(agent, game_num, Synchronizer, difficulty):
    ...
    blue_agent = DummyZerg(diff=difficulty)
    blue_agent.get_power()
...
def Worker(index, update_game_num, Synchronizer, cluster, model_path):
    ...
    for i in range(THREAD_NUM):
        agent = Zerg(agent_id=i, global_buffer=global_buffer, net=mini_net, restore_model=FLAGS.restore_model)
        agents.append(agent)
...
def Parameter_Server(Synchronizer, cluster, log_path, model_path, procs):
    ...
    mini_net = MiniNetwork(sess, index=0, summary_writer=summary_writer, rl_training=True, cluster=cluster,
                           ppo_load_path=FLAGS.restore_model_path, ppo_save_path=model_path)
    agent = Zerg(agent_id=-1, global_buffer=Buffer(), net=mini_net, restore_model=FLAGS.restore_model)
