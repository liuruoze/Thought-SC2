...
class SourceAgent(base_agent.BaseAgent):
    ...
    def mini_step(self, action):
        ...
        if action == ZergAction.Build_drone.value:
            ...
            creep_index = U.get_creep_mask_screen(self.obs, size=5)
    ...
    def mapping_source_to_mini_by_rule(self, source_state):
        ...
        simple_input[14] = source_state[16]  # self.extractor_nums
        simple_input[15] = source_state[20]  # self.queen_num
        simple_input[16] = source_state[21] # self.building_nums

        simple_input[17] = source_state[2] + source_state[4] # self.collected_minerals
        simple_input[18] = source_state[4] # self.spent_minerals
        simple_input[19] = source_state[3] + source_state[5]# self.building_nums
        simple_input[20] = source_state[5] # self.spent_gas
    ...
    def play(self, verbose=False):
        ...
        M.set_rally_point(self, C.rally_pos)
        while True:
            ...
            if self.policy_flag and (not self.is_end):
                ...
                # action_probs, v_preds = self.net.policy.get_action_probs(state_now, verbose=False)
                # action = np.argmax(action_probs)
                # #action = 3

                # print(ZergAction(int(action)).name)
                # time.sleep(0.5)
