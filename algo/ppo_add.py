        ...
        with tf.variable_scope(name):
            ...
            with tf.variable_scope('policy_net'):
                with tf.variable_scope('controller'):              
                    ...
                    # weighted sum
                    layer_3 = layer.dense_layer(self.obs_add, 64, "DenseLayer3", func=activation, initial_type='original')
                    layer_2 = (1. -  self.add_weight) * layer_2 + self.add_weight * layer_3

                    #self.tech_probs = layer.dense_layer(layer_2, act_space_array, "tech_output", func=tf.nn.softmax)
                    self.tech_probs = layer.output_layer(layer_2, act_space_array, add_act_space, "tech_output", func=tf.nn.softmax)
...
class PPOTrain:
    ...
    def train(self, obs, obs_add, tech_actions, gaes, rewards, v_preds_next):
    ...
    def train_value(self, obs, obs_add, gaes, rewards, v_preds_next):
    ...
    def train_with_all(self, obs, obs_add, tech_actions, gaes, rewards, v_preds_next):
    ...
    def ppo_train_dis(self, observations, obs_add, tech_actions, rewards, v_preds, v_preds_next,
        ...
        inp = [observations, tech_actions, gaes, rewards, v_preds_next, obs_add]
