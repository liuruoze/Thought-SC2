...
class Policy_net:
    ...
    def get_action(self, obs, obs_add, obs_map, verbose=True):
        ...
        if verbose:
            ...
            print("Value:", v_preds)
        return tech_act[0], np.asscalar(v_preds)


    def get_action_more(self, obs, obs_add, obs_map, verbose=True):
        tech_act_probs, tech_act, v_preds, layer_2, layer_3, layer_4, layer_5 \
            = self.sess.run([self.tech_probs, self.tech_act, self.v_preds, self.layer_2, self.layer_3, self.layer_4, self.layer_5], 
    ...
    def get_action_probs(self, obs, obs_add, obs_map, verbose=True):
        ...
        if verbose:
            ...
            print("Tech:", 'act_probs:', tech_act_probs, 'act:', tech_act)
            print("Value:", v_preds)
        return tech_act_probs[0], np.asscalar(v_preds)
    ...
    def get_map_variables(self):
        return tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, self.map_variable_scope)

    def get_trainable_variables(self):
        return tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, self.scope)

    def get_map_trainable_variables(self):
        return tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, self.map_variable_scope)
...
class PPOTrain:
    ...
    def train(self, obs, obs_add, obs_map, tech_actions, gaes, rewards, v_preds_next):
    ...
    def train_value(self, obs, obs_add, obs_map, gaes, rewards, v_preds_next):
    ...
    def train_with_all(self, obs, obs_add, obs_map, tech_actions, gaes, rewards, v_preds_next):
        _, total_loss, loss_clip, loss_vf, entropy = self.sess.run([self.train_op, self.total_loss, 
                                                                self.loss_clip, self.loss_vf, self.entropy], 
                                                                            feed_dict={self.Policy.obs: obs,
                                                                                   ...
                                                                                   self.v_preds_next: v_preds_next,
                                                                                   self.gaes: gaes})
        return total_loss, loss_clip, loss_vf, entropy
    ...
    def ppo_train_dis(self, observations, obs_add, obs_map, tech_actions, rewards, v_preds, v_preds_next,
        ...
        inp = [observations, tech_actions, gaes, rewards, v_preds_next, obs_add, obs_map]
        ...
        for epoch in range(self.epoch_num):
            for step in range(max_steps):
                ...
                if self.restore_model and self.update_count < 3:
                    ...
                    total_loss, loss_clip, loss_vf, entropy = self.train_with_all(obs=sampled_inp[0], obs_add=sampled_inp[5],
                                            obs_map=sampled_inp[6], tech_actions=sampled_inp[1],
                                            gaes=sampled_inp[2],
                                            rewards=sampled_inp[3],
                                            v_preds_next=sampled_inp[4])
                    if True:
