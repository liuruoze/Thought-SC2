...
import param as P
import mini_source_agent_add_map_bn_debug as mini_source_agent
...
flags.DEFINE_bool("image_debug", False, "Whether debug image for input.")
flags.DEFINE_bool("action_prob_debug", True, "Whether action_prob image for input.")
...
flags.DEFINE_integer("port_num", 15770, "the start port to create distribute tf")
...
flags.DEFINE_integer("ob_space_add", 0, "Add state space from thought game.")
flags.DEFINE_integer("act_space_add", 0, "Add action space from thought game.")
flags.DEFINE_bool("add_image", False, "Whether add image for input.")
flags.DEFINE_bool("partial_restore", True, "Whether use partial_restore.")
...
def Worker(index, update_game_num, Synchronizer, cluster, model_path, log_path):
    ...
    for i in range(THREAD_NUM):
        agent = mini_source_agent.MiniSourceAgent(index=i, global_buffer=global_buffer, net=Net,
                                                  ...
                                                  image_debug=FLAGS.image_debug, action_prob_debug=FLAGS.action_prob_debug,
