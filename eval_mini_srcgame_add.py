...
import param as P
import mini_source_agent_add as mini_source_agent

# from pysc2.env import sc2_env
from lib import my_sc2_env as sc2_env
from lib.replay_buffer import Buffer

from mini_network_add import MiniNetwork
...
flags.DEFINE_string("restore_model_path", "./model/20200811-223305_mini/", "path for restore model")
...
flags.DEFINE_integer("port_num", 4270, "the start port to create distribute tf")
flags.DEFINE_integer("max_iters", 30, "the rl agent max run iters")
...
# kill -9 `ps -ef |grep liuruoze | grep eval_mini_srcgame_add | awk '{print $2}' `
...
def Worker(index, update_game_num, Synchronizer, cluster, model_path):
    ...
    Net = MiniNetwork(sess=sess, summary_writer=None, rl_training=FLAGS.training,
                      ...
                      ob_space_add=FLAGS.ob_space_add, act_space_add=FLAGS.act_space_add, freeze_head=FLAGS.freeze_head)
