...
flags.DEFINE_string("restore_model_path", "./model/20200909-182113_dream/", "path for restore model")
...
flags.DEFINE_integer("port_num", 7370, "the start port to create distribute tf")
...
ITERATION_INDEX = "2"
net_path_name = "tf_models" + "_" + ITERATION_INDEX
...
def Parameter_Server(Synchronizer, cluster, log_path, model_path, procs):
    ...
    Net = MiniNetwork(sess=sess, summary_writer=summary_writer, rl_training=FLAGS.training,
                      ...
                      ppo_load_path=FLAGS.restore_model_path, ppo_save_path=model_path)
    Sec_Net = SecondNetwork(sess=sess, rl_training=False, reuse=True, cluster=None, index=0, 
        load_model=True, net_path_name=net_path_name)
