...
import mini_source_agent_dream as mini_source_agent
from mini_network_dream import MiniNetwork, SecondNetwork
...
flags.DEFINE_string("restore_model_path", "./model/20200907-171036_sourcedream/", "path for restore model")
...
flags.DEFINE_integer("port_num", 7470, "the start port to create distribute tf")
...
ITERATION_INDEX = "1"
DIR_NAME = 'record_img_dream' + "_" + ITERATION_INDEX
...
def Worker(index, update_game_num, Synchronizer, cluster, model_path):
    ...
    Sec_Net = SecondNetwork(sess=sess, rl_training=False, reuse=True, cluster=None, index=index, load_model=True)

    global_buffer = Buffer()
    agents = []
    for i in range(THREAD_NUM):
        agent = mini_source_agent.MiniSourceAgent(index=i, global_buffer=global_buffer, net=Net, sec_net=Sec_Net,
                                                  restore_model=FLAGS.restore_model, rl_training=FLAGS.training,
                                                  strategy_agent=None, extract_save_dir=DIR_NAME)
        agents.append(agent)
...
def Parameter_Server(Synchronizer, cluster, log_path, model_path, procs):
    ...
    Net = MiniNetwork(sess=sess, summary_writer=summary_writer, rl_training=FLAGS.training,
                      ...
                      ppo_load_path=FLAGS.restore_model_path, ppo_save_path=model_path)
    Sec_Net = SecondNetwork(sess=sess, rl_training=False, reuse=True, cluster=None, index=0, load_model=True)

    agent = mini_source_agent.MiniSourceAgent(index=-1, net=Net, sec_net=Sec_Net,
        restore_model=FLAGS.restore_model, rl_training=FLAGS.training)
...
def _main(unused_argv):
    ...
    now = datetime.now()
    model_path = "./model/" + now.strftime("%Y%m%d-%H%M%S") + "_sourcedream/"
    if not os.path.exists(model_path):
        os.makedirs(model_path)
    log_path = "./logs/" + now.strftime("%Y%m%d-%H%M%S") + "_sourcedream/"
