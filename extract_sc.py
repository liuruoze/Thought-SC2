...
flags.DEFINE_integer("parallel", 5, "How many processes to run in parallel.")
...
flags.DEFINE_integer("max_iters", 2, "the rl agent max run iters")
...
def run_thread(agent, game_num, Synchronizer, difficulty):
    ...
    with sc2_env.SC2Env(
        ...
        while all_num != game_num * TRAIN_ITERS:
            agent.sample(use_image=USE_IMG)
