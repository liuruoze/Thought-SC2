    ...
    def step_auto_2(self, verbose=False):
        ...
        if self.env.is_end:
            ...
            if self.state_last is not None:
                v_preds_next = self.policy.get_values(self.state_now, self.add_state)
                v_preds_next = self.get_values(v_preds_next)
                
                if False and final_reward != 0:
                    print("add buffer:", self.state_last, self.action_last, self.state_now, final_reward, self.v_preds, v_preds_next)

                self.local_buffer.append_more(self.state_last, self.add_state_last, self.action_last, self.state_now, 
                    final_reward, self.v_preds, v_preds_next)
        ...
        if self.state_last is not None:
            ...
            reward = 0
            self.local_buffer.append_more(self.state_last, self.add_state_last, self.action_last, self.state_now, 
                reward, self.v_preds, v_preds_next)

        self.action, self.v_preds = self.policy.get_action(self.state_now, self.add_state, verbose=verbose)
        self.state_last = self.state_now
        self.add_state_last = self.add_state
