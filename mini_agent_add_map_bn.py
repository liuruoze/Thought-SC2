...
class MiniAgent(A):
    ...
    def step_auto(self, verbose=False):
        self.step_auto_2(verbose)

    def step_auto_2(self, verbose=False):
        self.add_state = np.zeros(1)
        self.map_state = np.zeros([1, 1, 1])
        ...
        if self.env.is_end:
            ...
            if self.prev_state is not None:
                self.value = self.policy.get_values(self.state_now, self.add_state, self.map_state)
                self.value = self.get_values(self.value)
                reward = final_reward

                if False and final_reward != 0:
                    print("add buffer:", self.prev_state, self.prev_action, self.state_now, final_reward, self.value, value_next)

                self.local_buffer.append_more_more(self.prev_state, self.prev_add_state, self.prev_map_state, 
                    self.prev_action, self.state_now, 
                    reward, self.prev_value, self.value)
        ...
        self.action, self.action_probs, self.value = self.policy.get_act_action_probs(self.state_now, self.add_state, self.map_state, verbose=verbose)
        ...
        if verbose:
            ...
            #time.sleep(1)
            U.show_prob_dist(self.action_probs)
        ...
        self.prev_state = self.state_now
        self.prev_add_state = self.add_state
        self.prev_map_state = self.map_state
    ...
    def get_values(self, values):
        ...
        if self.env.is_end and self._result != 0:
            return 0
        else:
            return values

    def get_action_by_policy(self, obs):
        act, value = self.policy.get_action(obs, verbose=False)
        return act, value
