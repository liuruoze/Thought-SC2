...
class MiniSourceAgent(base_agent.BaseAgent):
    ...
    def __init__(self, index=0, rl_training=False, restore_model=False, global_buffer=None, net=None, strategy_agent=None, greedy_action=False,
        extract_save_dir=None, ob_space_add=0, image_debug=False, action_prob_debug=False, probe_debug=True, act_space_add=0):
        ...
        self.probe_debug = probe_debug
        self.edge_state_debug = False
    ...
    def play_right_add(self, verbose=False):
        ...
        while True:
            ...
            if self.policy_flag and (not self.is_end):
                ...
                if self.probe_debug and verbose and self.step % C.time_wait(self.prove_save_wait_secs) == 0:
                    save_prob = True
                else:
                    save_prob = False
                ...
                if self.action_prob_debug and verbose:
                    ...
                    if True and simulate_seconds in self.prob_show_wait_seconds:
                        ...
                        U.show_prob_dist(action_probs, show=True, color=color, max_y=max_y,
                            action_num=self.action_num, save=False, name=bar_name, count=0)
                ...
                if self.edge_state_debug and verbose:
                    ...
                    for i, match in enumerate(match_list):
                        if match and i != previous_match:
                            print('Match:', i+1)
                            match_name = 'ES' + '_' + str(i+1) + '_' + str(simulate_seconds)
                            U.show_prob_dist(action_probs, show=True, color=color, max_y=max_y,
                                action_num=self.action_num, save=True, name=match_name, count=0)
                            previous_match = i
