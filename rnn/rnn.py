...
class MDNRNN():
  ...
  def build_model(self, hps):
    ...
    INWIDTH = hps.input_seq_width # 62 channels
    OUTWIDTH = hps.output_seq_width # 52 channels
...
def sample_sequence(sess, s_model, hps, init_z, actions, temperature=1.0, seq_len=1000):
  ...
  for i in range(seq_len):
    input_x = np.concatenate((prev_x, actions[i].reshape((1, 1, ACTION_SPACE))), axis=2)
...
def rnn_next_state(rnn, z, a, prev_state, mode2=F_MODE_I_N):
  input_x = np.concatenate((z.reshape((1, 1, rnn_feature_size(mode2))), a.reshape((1, 1, ACTION_SPACE))), axis=2)
  feed = {rnn.input_x: input_x, rnn.initial_state:prev_state}
  return rnn.sess.run(rnn.final_state, feed)

def rnn_output_size(mode1, mode2=F_MODE_I_N):
  if mode1 == MODE_ZCH:
    return (rnn_feature_size(mode2)+256+256)
  if (mode1 == MODE_ZC) or (mode1 == MODE_ZH):
    return (rnn_feature_size(mode2)+256)
  return rnn_feature_size(mode2) # MODE_Z or MODE_Z_HIDDEN

def rnn_feature_size(mode):
  if mode == F_MODE_I_N:
