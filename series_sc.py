...
def load_raw_data_list(filelist):
  ...
  for i in range(len(filelist)):
    ...
    if ((i+1) % 1000 == 0):
      ...
      gc.collect()
  return obs_list, img_list, action_list
...
obs_dataset, img_list, action_dataset = load_raw_data_list(filelist)
gc.collect()
...
np.savez_compressed(os.path.join(SERIES_DIR, "series_10000.npz"), action=action_dataset, obs=obs_dataset, mu=mu_dataset, logvar=logvar_dataset)
