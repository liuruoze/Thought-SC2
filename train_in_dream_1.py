...
from mini_network_dream import MiniNetwork
from dream_agent import MiniAgent
from rnn.rnn_dream import ScRNNEnv
...
FLAGS = flags.FLAGS
flags.DEFINE_string("restore_model_path", "./model/20190121-210229_dream/", "path for restore model")
...
flags.DEFINE_integer("port_num", 5570, "Port number for distribute training in tensorflow")
flags.DEFINE_integer("train_iters", 100, "How many iterations for one training")
...
flags.DEFINE_string("type", "dream", "model type")
FLAGS(sys.argv)

ITERATION_INDEX = "1"
env_save_path = "tf_models" + "_" + ITERATION_INDEX
...
def run_thread(agent, game_num, Synchronizer, difficulty):
    ...
    env = ScRNNEnv(env_path_name=env_save_path)
    #env = ENV

    agent.set_env(env)

    while True:
        agent.play(SHOW_DETAILS)
...
if __name__ == "__main__":
    ...
    now = datetime.now()
    model_path = "./model/" + now.strftime("%Y%m%d-%H%M%S") + "_" + FLAGS.type + "/"
